...
#'
#' @description function makes a GET request to the Human Cell Atlas API using
#'   the provided url string
#'
#' @param url character(1) url string for API call
#'
#' @return list() response headers and content from call, error message outpt if
#'   needed
#' @export
hca_api_call <- function(url = ''){
    response <- httr::GET(url)
    tryCatch({
        ## make sure response code isn't 400 or higher

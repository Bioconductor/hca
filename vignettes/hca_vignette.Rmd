...
Use `projects()` to retrieve all projects in the HCA.
...
interested in, with title starting with "Single cell
trans...". Extract the unique `projectId` for this project.
...
Note that `files_download()` uses [BiocFileCache][], so individual
...
projects <- projects(filters, columns = columns, catalog = "dcp2")
projects
```
...
them. We illustrate pagination with `projects()`, retrieving only 30
of the 44 projects in the current DCP2 atlas.

Pagination works for the default `tibble` output

```{r}
page_1_tbl <- projects(size = 30, catalog = "dcp2")
page_1_tbl
...
```{r}
page_1_lol <- projects(size = 5, as = "lol", catalog = "dcp2")
page_1_lol %>%
...
```{r}
heart_filters <- filters(organ = list(is = "heart"))
summary(filters = heart_filters, type = "fileTypeSummaries", catalog = "dcp2")

summary(type = "overview", catalog = "dcp1")
```
...
projects_detail(uuid = projectId, catalog = "dcp2")
```

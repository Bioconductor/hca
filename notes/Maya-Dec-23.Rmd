...
Then the base URL, endpoint, and all other parameters
...
1. Figure out authentication (Martin)
    ...
    - This turned out to be a non-issue, the path we used originally
      was `/index/project` (singular), but should have been
      `/index/projects`. We had also used `order="as"` when we should
      have had `order="asc"`

2. Figure out how to parse return value (Maya)

    - What are the `projectTitle` of projects with 'pancreas' as
      organ? (should be same as [here][])
    
    - As a start, here are the 'names' of list elements, and the
      length (number of elements of each element
      ...
      Note that elements do not have to be named, e.g., each 'hit' is
      not named
      ...
      Unnamed elements can be accessed via a 1-based double square bracket
      
      ```{r}
      names(lst$hits[[1]])
      ```
      
      The path to the first project title matching the filter is (in
      two equivalent ways)
      ...
      Use `sapply()` with `X = lst$hits` to generate a vector of
      project titles matching our query.

3. What information in is `lst$pagination` and `lst$termFacets`?

4. Formulate additional filters, projects using `genusSpecies = "Homo
   sapiens"`. What about combining filters -- projects using `"Mus
   musculus"` (mouse) to study expression in the brain.

5. Incorporate simple queries like this into an R package.
